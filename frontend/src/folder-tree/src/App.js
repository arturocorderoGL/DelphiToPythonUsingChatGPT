...
const App = () => {
  ...
  useEffect(() => {
    const loadInitialTree = async () => {
      const response = await axios.get(`http://ec2-52-90-95-78.compute-1.amazonaws.com/loadtree/${repoOwner}/${repoName}`);
      const data = {
        name: repoName,
        type: 'dir',
  ...
  const handleFileClick = async (fileNode) => {
    ...
    const response = await axios.post(
      'http://ec2-52-90-95-78.compute-1.amazonaws.com/loadfile',
      data,
  ...
  const fetchLongTask = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://ec2-52-90-95-78.compute-1.amazonaws.com/execute');
